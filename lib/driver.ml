...
let initial_coqfile = "temp_quickchick_ocaml.v"
let final_coqfile = "quickchick_ocaml.v"
...
let process_translations filename (info : Required.t) =
  List.concat_map info.extractions ~f:(fun ex ->
      match ex with
      | Ty { type_name; _ } -> Files.get_translated_type filename type_name
      | Func { function_name; _ } ->
          Files.get_axiomatized_function filename function_name
      | File _ -> [])
...
let quickchick_arg (info : Required.t) =
  ...
  Cmd.run_coqc info.temp_dir post_coqfile |> Files.find_time_elapsed |> fun s ->
  match s with
  | Some s -> Printf.printf "Found a bug in %f seconds!" s
  | None -> Printf.printf "Did not find a bug"
